...
{
    public UndirectedGraph() {this.graph = new HashMap<>();}
    public UndirectedGraph(HashMap<String, List<String>> graph) {setGraph(graph);}
    public UndirectedGraph(UndirectedGraph uGraph)
    {
        if (uGraph.getStatus()) this.graph = new HashMap<>(uGraph.getGraph());
        else System.out.println("Переданный вами в конструктор граф был некорректно создан, пресоздайте его!");
    }
    public UndirectedGraph(String wayToFile) {setGraph(readGraphFromFile(wayToFile));}
    ...
    public void setGraph(HashMap<String, List<String>> graph)
    {
        ...
        if (!err)
        {
            ...
            for (String key : this.graph.keySet()) for (String edge : this.graph.get(key)) if (!this.graph.get(edge).contains(key)) this.graph.get(edge).add(key);
        }
    }
        ...
        if (this.isNormal)
        {
            if (!this.graph.containsKey(name))
            {
                ...
                if (!err)
                {
                    ...
                    for (String edge : edges) if (!this.graph.get(edge).contains(name)) this.graph.get(edge).add(name);
                }
