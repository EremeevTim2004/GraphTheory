...
{
    public OrientedWeightedGraph() {this.graph = new HashMap<>();}
    public OrientedWeightedGraph(HashMap<String, List<String>> graph) {setGraph(graph);}
    public OrientedWeightedGraph(OrientedWeightedGraph oWGraph)
    {
        if (oWGraph.getStatus()) this.graph = new HashMap<>(oWGraph.getGraph());
            ...
            if (this.graph.containsKey(name))
            {
                ...
                for (String vertex : this.graph.keySet())
                {
                    for (String connect : this.graph.get(vertex))
                    {
                        ...
                        if (edge.equals(name)) this.graph.get(vertex).remove(this.graph.get(vertex).indexOf(connect));
                    }
                }
    ...
    public void addEdge(String vertex, String edge)
    {
        if (this.isNormal)
        {
            ...
            if (this.graph.containsKey(vertex) && !err) this.graph.get(vertex).add(edge);
        }
    }
